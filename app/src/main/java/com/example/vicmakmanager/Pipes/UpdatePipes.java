...
import com.example.vicmakmanager.Grills.UpdateGrills;
...
public class UpdatePipes extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_pipes);
        ...
        update.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(buying_price.getText().toString().trim().isEmpty() || selling_price.getText().toString().trim().isEmpty() ||
                    ...
                    Toast.makeText(UpdatePipes.this, "Cannot submit blank fields", Toast.LENGTH_SHORT).show();
                }else{
                    databaseReference.child("VicmakStock").child("Pipes").addListenerForSingleValueEvent(new ValueEventListener() {
                        CountDownLatch done = new CountDownLatch(1);
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            databaseReference.child("VicmakStock").child("Pipes").child("buying_price").setValue(buying_price.getText().toString().trim());
                            databaseReference.child("VicmakStock").child("Pipes").child("selling_price").setValue(selling_price.getText().toString().trim());
                            databaseReference.child("VicmakStock").child("Pipes").child("stock").setValue(stock.getText().toString().trim());

                            int commission = Integer.parseInt(selling_price.getText().toString().trim()) - Integer.parseInt(buying_price.getText().toString().trim());

                            databaseReference.child("VicmakStock").child("Pipes").child("commission").setValue(commission + "");
                            ...
                            databaseReference.child("VicmakStock").child("Pipes").child("total_commission").setValue((sold * commission) + "");

                            Toast.makeText(UpdatePipes.this, "Updated Successfully", Toast.LENGTH_SHORT).show();
                        }
